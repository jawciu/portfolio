// Shared read-only primitives for the Gateway case study sections (cloned from the
// wiki-whisperer kit). Builders IMPORT from here for consistency — do not edit per-section.
...
export const A = (file: string) => `/projects/gateway/${file}`;
...
export function PlaceholderShot({
  ...
  return (
    <div
      ...
      className="flex w-full items-center justify-center rounded-2xl border-2 border-dashed border-[var(--cog-line)] bg-[var(--cog-bg-alt)]"
    >
      <p className="font-[family-name:var(--font-mono)] px-6 text-center text-[13px] uppercase tracking-[0.12em] text-[var(--cog-muted)]">
        {label}
      </p>
    </div>
  );
